...
if sys.version_info < (3,):
    ...
    PUNCTUATION_CHARACTERS = string.punctuation.translate(
        str.maketrans('', '', '-_.'))
...
class DBAccessParser:
    ...
    def __init__(self):
        self.table = re.compile("\<table name=\"(\S+)\".*\>")
        self.view = re.compile("\<view name=\"(\S+)\".*\>")
        self.complex_type = re.compile("\W+\<complex_type name=\"(\S+)\".*\>")
        self.function = re.compile("\W+\<function name=\"(\S+)\".*\>")
        return

    def parse(self, path):
        # hfile.write("Parsing DBAccess {}\n".format(path))
        result = {}
        f = open(path, "r")
        cur_line = 0
        for line in f:
            cur_line = cur_line + 1
            match = self.function.match(line)
            if match:
                name = match.group(1)
                # hfile.write("function {}\n".format(name))
                result[(True, name, cur_line)] = name
                result[(True, name + "_begin", cur_line)] = name
                result[(True, name + "_next", cur_line)] = name
                result[(True, name + "_end", cur_line)] = name
            else:
                match = self.complex_type.match(line)
                if match:
                    name = match.group(1)
                    # hfile.write("complex_type {}\n".format(name))
                    result[(True, name, cur_line)] = name
                else:
                    match = self.table.match(line)
                    if match:
                        name = match.group(1)
                        # hfile.write("table {}\n".format(name))
                        result[(True, name, cur_line)] = name
                    else:
                        match = self.view.match(line)
                        if match:
                            name = match.group(1)
                            # hfile.write("view {}\n".format(name))
                            result[(True, name, cur_line)] = name
...
class APIErrorsParser:
    def __init__(self):
        self.deferr = re.compile(
            "\W+\<def_error name=\"(\S+)\" value=\"(\S+)\"\>")
        self.err = re.compile(
            "\W+\<error name=\"(\S+)\".*\>")
        return

    def parse(self, path):
        # hfile.write("Parsing API ERRORS {}\n".format(path))
        result = {}
        f = open(path, "r")
        cur_line = 0
        for line in f:
            cur_line = cur_line + 1
            match = self.deferr.match(line)
            if match:
                name = match.group(1)
                # value = match.group(2)
                # hfile.write("def_err {} {}\n".format(name, value))
                result[(True, name, cur_line)] = name
            match = self.err.match(line)
            if match:
                name = match.group(1)
                # hfile.write("err {}\n".format(name))
                result[(True, name, cur_line)] = name
...
class APIFunctionsParser:
    def __init__(self):
        return

    def parse(self, path):
        # hfile.write("Parsing API Functions {}\n".format(path))
        result = {}
        return result
...
class CtagsParser:
    def __init__(self, ctags_command, options):
        self.process = subprocess.Popen(
            [ctags_command, '-xu', '--filter',
             '--filter-terminator=' + TERMINATOR, '--format=1'],
            bufsize=-1,
        ...
        if sys.version_info < (3,):
            ...
            self.child_stdout = io.TextIOWrapper(
                self.process.stdout.buffer, encoding='latin1')
...
def select_parser(path, default_parser):
    # hfile.write("path {}\n".format(path))
    if "apierrors.xml" in path:
        # hfile.write("apierrors.xml in path {}\n".format(path))
        return APIErrorsParser()
    elif "apifunctions.xml" in path:
        return APIFunctionsParser()
    elif "dbaccess" in path and ".xml" in path:
        # hfile.write("dbaccess in path {}\n".format(path))
